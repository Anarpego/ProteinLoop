...
The first operational view is `Live tank simulation`, built with pinned Three.js `0.185.1`. It loads the Khronos Barramundi Fish PBR glTF model from the local application, clones shared geometry and textures across the school, and falls back to a muted procedural fish if loading fails. The bundled model is CC0 and its provenance and checksum are recorded in [`BARRAMUNDI-LICENSE.md`](app/priv/static/models/BARRAMUNDI-LICENSE.md). Four foreground prawns use a locally bundled CC0 Greasyback Shrimp visual whose black source background is removed at render time; the segmented 3D prawns remain as the loading/error fallback. Its provenance is recorded in [`GREASYBACK-SHRIMP-LICENSE.md`](app/priv/static/models/GREASYBACK-SHRIMP-LICENSE.md). Bubbles, varied substrate, plants, physical water and glass, soft shadows, and water-loop piping complete the scene. Phoenix patches simulator values every second without replacing the canvas. Ammonia changes water clarity and color; dissolved oxygen changes bubble activity, swimming speed, and whether fish move toward the surface. The browser Fullscreen API expands the same live renderer without remounting it. In operator mode, the immersive overlay retains emergency/reset controls, readable chemistry, mission selection, the real Sagents run event, verifier status, and human-approval boundary. The producer route reuses the same full-screen animation in read-only mode. A light non-illustrated layer and readable HTML chemistry remain available if WebGL cannot start.
