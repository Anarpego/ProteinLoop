...
Act-II organizers confirmed that ProteinLoop was assigned a team Jupyter pod at `https://notebooks.amd.com/hackathon` and that its optional Fireworks coupon was emailed. They also clarified that AMD Developer Cloud is not an Act-II compute offering. Successful notebook startup and coupon redemption have not yet been captured as executable project evidence, so the submitted demo continues to use owned infrastructure: self-hosted Gemma 4 E2B through llama.cpp on an 8 GB DigitalOcean CPU host, plus separate Apple Metal evidence. The repository includes a tested AMD ROCm/vLLM profile behind the same `GEMMA_ENDPOINT` and an access preflight for either official remote path, but does not misrepresent the current public demo as AMD-hosted or GPU-backed.
